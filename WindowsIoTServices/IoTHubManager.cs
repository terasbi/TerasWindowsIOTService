    ...
    public static class IoTHubManager
    {
        ...
        private static async Task GetDeviceAsync(string deviceId)
        {
            ...
            try
            {
                ...
                SaveConnectionString(IoTMainClass.StrIotHubConnString.Split(';')[0].Substring(9), deviceId, device.Authentication.SymmetricKey.PrimaryKey);
                SaveConnectionString(IoTMainClass.StrIotHubConnString.Split(';')[0].Substring(9), deviceId, device.Authentication.SymmetricKey.SecondaryKey);
            }
